...
/** economy_remove &lt;amount&gt; — removes coins from the player's purse. */
public class EconomyRemoveEventFactory implements EventFactory {

    private final EconomyModule module;

    public EconomyRemoveEventFactory(EconomyModule module) {
        this.module = module;
    }
    ...
    public CompiledEvent compile(String[] args, EventOptions options) {
        ...
        return context -> context.getPlayerCaster().map(Player::getUniqueId).ifPresent(uuid -> {
            ...
            if (amount > 0) module.removePurse(uuid, amount);
        });
    }
