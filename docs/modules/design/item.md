...
`updateLore(ItemStack)` re-regenerates lore in place; `updateLore(item, meta)` composes eight independent line-list blocks — breaking-power, base lore, lore-template (with `$item.stat.<id>$` token resolution via `resolveItemStatTokens`), stats, modifiers (attached LORE-format modifiers e.g. gemstones), enchantments (via `EnchantmentHelper.formatEnchants`), abilities, and the rarity tag — each keyed by an `ItemLoreLayout.Section` enum constant, then stitches them together in the order (and with the per-block MiniMessage formats) read fresh from `config.yml`'s `items.lore` on every call via `ItemLoreLayout.load(plugin)` (**not** cached — this isn't a hot path, and it means `/valmora reload`, which now also calls `plugin.reloadConfig()`, picks up edits immediately). A block whose line list ends up empty is skipped entirely (no stray spacer); `spacer-between-sections` (default `true`) controls whether one blank line is inserted between consecutive non-empty blocks. See `docs/modules/user/item.md` §17 for the full per-block config reference. `getBreakingPower` derives breaking power from the material/tool.
...
- **`AbilityDefinition`** (`AbilityDefinition.java`): id, name, trigger, targetRange, cooldown, manaCost, description, `displayMode` (`DisplayMode.FULL`/`SIMPLE`, YAML key `display`, default `FULL`), conditions (list of script expressions), mechanics (list of `ConfiguredMechanic`).
