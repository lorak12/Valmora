...
- ~~**No live output preview.**~~ **Resolved 2026-08-07** — the no-player `match(inputs)` overloads for `reforge_anvil`/`forge_random` used to hardcode `Optional.empty()`; they now delegate to the same private matchers as the player overload, with `player = null`. `checkAndNotifyCoins(null, ...)` already treated a null player as "always affordable" (nothing to notify), so this only had to stop the handler from bailing out before reaching it — no coin check/consumption happens during a preview either way, since that only occurs in the returned `RecipeDefinition`'s consume callback, which the preview path never invokes. `reforge_anvil.yml`'s `OUTPUT` component now shows the would-be reforged item as soon as both inputs are placed. `reforge.yml` (forge_random) has no `OUTPUT` slot to begin with (see §3.1), so it gets no visible benefit from this fix, and its preview — when it *is* re-queried, e.g. on `on-update` — re-rolls independently each time since the match itself is randomized, so it's an example-of-possible-outcomes preview, not a guaranteed-outcome one.
