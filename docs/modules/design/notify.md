...
1. ~~**Quest-package notification categories never load (load-order defect).**~~ **Fixed (2026-08-07).** `notifyModule` now registers before `questModule` in `Valmora.java`.
2. ~~**`AdvancementIO` is a stub.**~~ **Fixed (2026-08-07).** Implements real toasts via the standard dynamic-fake-advancement technique (`Bukkit.getUnsafe().loadAdvancement`/`removeAdvancement` — a supported public API, no NMS/packets needed after all), honoring `frame` and `icon`.
3. ~~**`SoundIO`... invalid `sound:` key throws**~~ **Fixed (2026-08-07).** Ignoring the message text is still intentional/expected; an invalid `sound:` key now logs a warning and no-ops instead of throwing.
4. ~~**`BossBarIO`... hide task is not cancelled on player quit.**~~ **Fixed (2026-08-07).** Pending auto-hide tasks are now tracked per player and cancelled/hidden on `PlayerQuitEvent` via a new `NotifyQuitListener`. (Still uses the global scheduler, not the entity scheduler — a smaller, non-blocking style nit left as-is.)
5. **Documented features that do not exist in code — decision (2026-08-07): drop, except `advancement` frame/icon (done above).** `docs/QUEST_MODULE_OUTLINE.md:459-568` describes several BetonQuest-inspired extras. Disposition: comma-separated category lists, sound-options-on-any-IO, and the `duration` key are cheap and plausibly worth adding later but are not blocking anything real today — left as documented-but-unimplemented. `totem` IO, `bossbar` `barFlags`/`countdown`, and `soundlocation`/`soundplayeroffset` are explicitly dropped — `QUEST_MODULE_OUTLINE.md`'s own header already flags them as "never built" reference material, not a spec, and no shipped content needs them.
...
7. ~~**Shipped `notify chat …` syntax prints the literal word "chat".**~~ **Fixed (2026-08-07).** `pets/baby_wolf.yml` now uses `notify <message> io:chat` throughout.
