...
- **Player victim** (`DamageApplier.java:21-39`): requires a loaded session (`ValmoraPlayer`); otherwise logs a warning and **returns without applying damage** (`DamageApplier.java:23-27`). Reads the active profile, then:
  - **`TotemProtectionService.tryProtect(player, state, statManager, finalDamage)`** — VANILLA_CONTROL_AUDIT.md §9 fix, checked *before* the health reduction below. Vanilla's own totem-death-protection check lives inside NMS's damage-application path, which never runs here (real `EntityDamageEvent` damage is always zeroed by `CombatListener` and death is force-triggered by a direct `player.setHealth(0)` call — see the bullet below — bypassing that check entirely). If the hit would be fatal and the player holds a Totem of Undying (either hand), this fires a real `EntityResurrectEvent` (so other plugins observing that event still see/can cancel it), consumes one totem, sets virtual health to the vanilla-equivalent 1-HP-of-20 sliver, applies the vanilla totem potion effects, and returns `true` — in which case `DamageApplier` **skips** the reduce/sync steps below entirely (the totem service already called `syncVisualHealth` itself). See `TotemProtectionService`'s class doc and `docs/modules/design/death.md`.
  - Otherwise: `state.reduceHealth(finalDamage)` — subtracts from **virtual** health (`PlayerState.reduceHealth`, `PlayerState.java:52-54`).
  - `syncVisualHealth(player, state, statManager)` — maps virtual health onto a fixed **10-heart (20 HP) scale** and pushes it to the client (`PlayerManager.syncVisualHealth`, `PlayerManager.java:243-267`); if virtual health hits `<= 0` it calls `player.setHealth(0)` to trigger a real vanilla death — which the `death` module (VANILLA_CONTROL_AUDIT.md §9, `docs/modules/design/death.md`) then owns via `PlayerDeathEvent`/`PlayerRespawnEvent`.
